...
Component({
  ...
  methods: {
    ...
    formatMonthData(date) {
      const value = dateUtil.getTotalDays(date)
    ...
    formatMultipleDateColor(date) {
      ...
      if (this.properties.selectDateMultiple.length !== 0) {
        // 判断预选的日期是否与当前的日期一致
        this.properties.selectDateMultiple.forEach((item) => {
          date.list.forEach((arr) => {
    ...
    formatRangeDateColor(date) {
      // 格式化开始的日期和结束日期时间戳
      const beginTimeStamp = new Date(this.properties.selectDateRange.begin.replace(/-/g, '/')).getTime()
    ...
    calculateResidualDays(date) {
      ...
      const lastValue = dateUtil.getTotalDays({
        year: date.year,
        month: date.month - 1
      })
      for (let i = 0; i < date.firstDayWeek; i++) {
        date.list.unshift({
          value: this.properties.showRemnantDays ? lastValue - i : '',
          type: 'last'
        })
      }
      ...
      const nextValue = total * 7 - date.list.length
    ...
    // 计算下个月的日期数据
    getNextMonth() {
      const changeKey = DAY_TYPE[(this.data.showCalendarIndex + 1) % 3]
      const date = this.data.calendarInfo[DAY_TYPE[this.data.showCalendarIndex]]
      let changeDate = this.data.calendarInfo[changeKey]
    ...
    selectDate(e) {
      ...
      // 修改对应的月份
      const date = this.data.calendarInfo[key]
    ...
    selectDateRange(key, index) {
      ...
      if (this.properties.selectDateRange.begin === '') {
        ...
        if (beginTimeStamp > endTimeStamp) {
          // 重新设置开始位置
          this.properties.selectDateRange = Object.assign(
            {},
        ...
        // 判断起始和结束的天数是否大于选择的范围
        const dayRange = this.properties.allowSameDate ? 1 : 2
        if (this.properties.selectDateRangeSize >= dayRange) {
          // 判断起始和结束相差的天数,要加上结束那一天
          const differDay = (endTimeStamp - beginTimeStamp) / (24 * 60 * 60 * 1000) + 1
          if (differDay > this.properties.selectDateRangeSize) {
            ...
            // 计算结束的位置
            const differTimeStamp = (this.properties.selectDateRangeSize - 1) * 24 * 60 * 60 * 1000
    ...
    formatShowTip(date) {
      ...
      // 循环遍历找出对应的要显示的文字日期
      this.properties.dateText.forEach((item) => {
        date.list.forEach((arr) => {
          const value = date.year + '-' + date.month + '-' + arr.value
    ...
    formatHolidayTip(date) {
      holiday.holidayList.forEach((item) => {
        date.list.forEach((arr) => {
